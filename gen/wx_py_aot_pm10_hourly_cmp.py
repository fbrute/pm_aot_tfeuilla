...
class MyNavigationToolbar(NavigationToolbar2WxAgg):
    ...
    def prepdata(self):
        ...
        self.ntyear, self.ntmonth, self.ntday,self.ntjul_day, self.nthour, self.ntpm10,self.ntaot1020 = range(7) 
        #self.tot_array_data3 = mlab.load("aot_pm_daily.txt",comments="#", delimiter=",")
        self.tot_array_data3 = mlab.load("aot_pm_hourly.txt",comments="#", delimiter=",")
	# Année à garder
	self.nyear = 2006
	...
	self.aot_pm = []
        ...
        vaot1020 = self.aot_pm[:,self.ntaot1020]*100
	...
	xdeb = 30
	ax.text(xdeb,40,("nombre de points = %d" % len(vaot1020)))
	ax.text(xdeb,20,("origine = %f" % origine))
	ax.text(xdeb,10,("coeffreg = %f" % coeffreg))

        interlinear = pente*svaot1020 + origine
        ax.plot(svaot1020,interlinear,'-b')

        pente = pente * 100.0
	ax.text(xdeb,30,("pente = %f" % pente))
        ax.set_title("PM10 (horaire) fonction de l' AOT1020*100 (%d,v15 apres selection)" % self.nyear,fontsize=8)
	print "pente=%f" % pente
...
class CanvasFrame(wx.Frame):
    ...
    def __init__(self):
        wx.Frame.__init__(self,None,-1,
                         'wx_py_aot_pm10_hourly_cmp',size=(550,350))
