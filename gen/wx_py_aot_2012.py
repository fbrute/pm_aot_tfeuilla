...
class MyNavigationToolbar(NavigationToolbar2WxAgg):
	...
	vaot1020 = self.vaot1020
        import pylab as plt
        from numpy.random import normal
	#gaussian_numbers = normal(size=1000)
	#print gaussian_numbers
	bins_values = [value/100. for value in range(0,63,3)]
	...
	n,xout,patches = plt.hist(vaot1020,bins=bins_values)
	...
	plt.figure()
	plt.bar(xout[:20],[float(number)/sum(n) for number in n],width = 0.03)
	#print "patches =" , [patch for patch in patches]
        plt.title("Frequence AOT - 2012")
        plt.xlabel("Valeur")
